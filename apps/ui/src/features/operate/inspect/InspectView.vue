...
import InspectCatalogDrawer from './InspectCatalogDrawer.vue';
import {
  INSPECT_STEPS,
...
import { bff, describeApiError, type InspectCatalogEntry, type InspectServerTimeResponse } from '@/api/client';
...
import { buildForeignMqeEntity, makeForeignMetric } from './inspectForeign';
import { scopeShort, type ForeignForm, type Source } from './inspectTypes';
...
// ─── Catalog drawer ────────────────────────────────────────────────
// The browse / filter / foreign-form state lives in InspectCatalogDrawer;
// the parent owns the board, so commits flow back by name (catalog) or
// spec (foreign) and the parent creates the widgets + eager-resolves.

const drawerOpen = ref(false);

/** Names already on the board — passed to the drawer to dedupe staged
 *  foreign metrics. */
const widgetNames = computed(() => new Set(widgets.value.map((w) => w.metric.name)));

function openDrawer() {
  drawerOpen.value = true;
}

/** Commit catalog picks from the drawer. Clamp to the board cap and skip
 *  duplicates, then eager-resolve so operators get "show me top-N" without
 *  an extra click. */
function commitCatalog(names: string[]): void {
  const added: Widget[] = [];
...
/** Commit staged foreign metrics from the drawer — identical bulk-add
 *  semantics to the catalog tab. */
function commitForeign(specs: ForeignForm[]): void {
  const added: Widget[] = [];
  ...
  <div class="ins">
    ...
    <transition name="drawer">
      <InspectCatalogDrawer
        ...
        :board-cap="boardCap"
        :existing-names="widgetNames"
        @close="drawerOpen = false"
        @commit-catalog="commitCatalog"
        @commit-foreign="commitForeign"
      />
    </transition>
  </div>
</template>
...
/* Drawer slide-in. The transition classes land on InspectCatalogDrawer's
 * root (.drawer, which carries the backdrop); the sliding panel is a
 * descendant inside the child, so it's reached through :deep. */
.drawer-enter-from :deep(.drawer__panel), .drawer-leave-to :deep(.drawer__panel) { transform: translateX(100%); }
.drawer-enter-active :deep(.drawer__panel), .drawer-leave-active :deep(.drawer__panel) { transition: transform 180ms ease; }
