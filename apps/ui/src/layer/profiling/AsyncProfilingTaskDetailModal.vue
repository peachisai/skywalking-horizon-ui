...
-->
<!--
  Read-only task-detail modal for the async-profiler (Java) tab — the
  trace-profiling tab's counterpart. Shows the task's create parameters
  (events, instance fan-out, duration, exec args) plus the per-instance
  operation logs OAP fanned the task out to. Opening it must not disturb
  the parent's selected-task / instance-filter state, so it is driven by
  its own `task` ref in the parent. `fmtTime` is injected so the timestamp
