...
-->
<!--
  Read-only task-detail modal for the trace profiling tab: the task's create
  parameters plus the per-instance operation logs OAP fanned the task out to.
  Opening it must not disturb the parent's selected-task / segment state, so it
  is driven by its own `task` ref in the parent. `fmtTime` is injected so the
