    ...
    <section class="sw-card k8s">
      ...
      <div class="counts">
        <div v-for="k in counts" :key="k.label" class="count">
          <span class="count-value">{{ formatValue(kpiValues[k.label]) }}</span>
      ...
      <div class="usages">
        <div v-for="k in usages" :key="k.label" class="usage">
          <div class="usage-head">
            ...
            <span class="usage-value">{{ formatValue(kpiValues[k.label], '%') }}</span>
          </div>
          <div class="bar">
...
.k8s { display: flex; flex-direction: column; padding: 12px 14px; gap: 12px; min-height: 0; height: 100%; }
...
.count-label {
  font-size: 10px; color: var(--sw-fg-3); text-transform: uppercase; letter-spacing: 0.05em;
}
.usages { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.usage-head { display: flex; justify-content: space-between; margin-bottom: 4px; }
